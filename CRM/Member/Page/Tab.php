...
class CRM_Member_Page_Tab extends CRM_Core_Page {
 ...
 SELECT COUNT(m.id)
  ...
  WHERE m.owner_membership_id = {$dao->id} AND m.is_test = 0 AND ms.is_current_member = 1 AND ct.is_deleted = 0";
      ...
      // [EXTENSION][Membershiprecord]
      $mterms = $this->_getTerms($membership[$dao->id]);
      $all_mterms = array_merge($all_mterms, $mterms);
      // $this->assign('membership'.$dao->id.'_terms', $mterms);
    }
    $this->assign('membershipTerms', $all_mterms);
  ...
  /**
   * List the terms/periods of a specific membership of this member.
   * [EXTENSION][Membershiprecord]
   *
   * @param array $membership_details
   *   fields of the membership for narrowing the terms table query
   * @return array with the membership's terms to be listed
   */
  private function _getTerms($membership_details) {
    if(empty($membership_details) || !isset($membership_details['membership_id']))
      return;

    $q_sql = <<<SQL
SELECT `id`, `contribution_id`, `start_date`, `end_date`
FROM `civicrm_membership_term`
WHERE `contact_id` = %0 AND `membership_id` = %1
SQL;
    $q_params = array(
      array(intval($this->_contactId), 'Integer'),
      array(intval($membership_details['membership_id']), 'Integer')
    );
    $dao_results = CRM_Core_DAO::executeQuery($q_sql,$q_params);
    $terms = array();
    while ($dao_results->fetch()) 
      $terms[] = array(
        "id"=>$dao_results->id,
        "membership_id"=>$membership_details['membership_id'],
        "contribution_id"=>$dao_results->contribution_id,
        "start_date"=>date_create($dao_results->start_date)->format('Y-m-d'),
        "end_date"=>date_create($dao_results->end_date)->format('Y-m-d')
      );
    return $terms;
  }
