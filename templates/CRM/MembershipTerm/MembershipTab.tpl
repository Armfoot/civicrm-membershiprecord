{*
 ...
 +--------------------------------------------------------------------+
 | MODIFIED FOR Membershiprecord extension.                           |
 |                                                                    |
 +--------------------------------------------------------------------+
...
<p class="warning">{ts}Membership Record extension added details of terms/periods of each membership in this page.{/ts}</p>
...
{elseif $action eq 16} {* Browse memberships for a contact *}
     ...
     * POSSIBLE POP-UP URL for Memberships terms 
    {capture assign=termsURL}{crmURL p="civicrm/membershiprecord/view" q="reset=1&action=list&cid=`$contactId`&context=membership"}{/capture}
    *}
    ...
    {* [EXTENSION][Membershiprecord]
     * POSSIBLE pop-up buttons for showing list of specific terms
    <div id="membershipterms">
        <h3>{ts}Memberships Terms{/ts}</h3>
        <a accesskey="N" href="{$termsURL}" class="button"><span><i class="crm-i fa-list"></i> {ts}View all{/ts}</span></a>
        <a accesskey="N" href="{$termsURL}" class="button"><span><i class="crm-i fa-list"></i> {ts}View active{/ts}</span></a>
        <a accesskey="N" href="{$termsURL}" class="button"><span><i class="crm-i fa-list"></i> {ts}View inactive{/ts}</span></a><br /><br />
    </div>
    *}
        ...
        <table id="active_membership" class="display">
            ...
            <tr id="crm-membership_{$activeMember.id}" class="{cycle values="odd-row,even-row"} {$activeMember.class} crm-membership">
                <td class="crm-membership-membership_type">
                    [ID {$activeMember.membership_id}] {$activeMember.membership_type}
    ...
    {* [EXTENSION][Membershiprecord] *}
    <hr/>
    {include file="CRM/MembershipTerm/jsautoscroll.tpl"}
    {include file="CRM/MembershipTerm/TermsList.tpl"}
    {/if}
    ...
    <th></th>
            ...
            <tr id="crm-membership_{$inActiveMember.id}" class="{cycle values="odd-row,even-row"} {$inActiveMember.class} crm-membership">
                <td class="crm-membership-membership_type">
                    [ID {$activeMember.membership_id}] {$inActiveMember.membership_type}
